// MeetStream Labs: realtime-mode MIA agent.
//
// 1. Save a realtime MIA config (one speech-to-speech model) with POST /mia.
...
import { assertRealtime, buildRealtimeAgentConfig, describeAgent } from './src/agent.js';
...
function loadConfig() {
  return {
    ...
    botName: optional('BOT_NAME', 'MIA Realtime Agent'),
...
async function main() {
  ...
  if (config.existingAgentConfigId) {
    ...
    const agentConfig = buildRealtimeAgentConfig();
    const created = await createAgent(config.apiKey, agentConfig);
    state.agentConfigId = created.agentConfigId;
    console.log(`[ok] Created realtime agent ${state.agentConfigId}`);
  ...
  console.log('     Admit it from the waiting room, then just talk. Ctrl+C to stop.');
