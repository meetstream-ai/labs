// MeetStream Labs: wake-word-gated meeting assistant.
//
// 1. Save a pipeline MIA config with a `wake_word` block via POST /mia.
// 2. Send a bot into a meeting with that config attached.
// 3. The agent stays silent until someone says a wake phrase, then answers.
// 4. Ctrl+C removes the bot and, optionally, deletes the agent.
//
...
import { assertGateEnabled, buildWakeWordAgentConfig, describeAgent } from './src/agent.js';
...
function loadConfig() {
  return {
    ...
    botName: optional('BOT_NAME', 'Acme Assistant'),
...
async function main() {
  ...
  if (wakePhrases.length) {
    console.log(`     Say "${wakePhrases[0]}" followed by your question. Ctrl+C to stop.`);
  }
