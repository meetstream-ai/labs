...
- **Recording defaults are product policy, not API defaults.** Video is off unless you ask for it: every template sends `video_required: false` explicitly, because the REST API treats an omitted `video_required` as **true**. When video is on, the payload also carries `recording_config.video_layout: "speaker_view"`, because `grid_view` is the API default and speaker view is what people want for review and clipping; grid is used only when you ask for it. And no template sets `video_separate_streams` unless per-participant video is the point of it, which today means only [per-participant-video-recorder](./per-participant-video-recorder) (per-participant **audio** is a separate switch and is unaffected). The exceptions that keep video on by design are [video-recording-downloader](./video-recording-downloader), [per-participant-video-recorder](./per-participant-video-recorder) and [screenshot-capture](./screenshot-capture), whose screenshots are cut from the video capture.
